...
function portal(role){let d=roles[role],st=loadState();if(role==="Owner"){d={...d,summary:money(st.sales)+" Today"}}app.innerHTML='<div class="shell"><section class="card portal-orbit-only"><button class="back floating-back">← Roles</button><div class="portal-label"><small>SME COMMAND CENTER</small><h2>'+role+' Portal</h2></div><div class="orbit"><button class="center center-action" id="orbitCenter" type="button"><div><strong>'+d.center+'</strong><br><span>'+d.summary+'</span><em>Tap for snapshot</em></div></button>'+d.modules.map((m,i)=>'<button class="module" data-m="'+m+'" style="'+pos(i,d.modules.length)+'">'+m+'</button>').join("")+(role==="Owner"?'<button id="auditOrbit" type="button" aria-label="New Audit Trail" style="position:absolute;left:50%;bottom:-8px;transform:translateX(-50%);z-index:20;background:#00d9ff;color:#00131a;border:5px solid #fff;border-radius:999px;padding:12px 20px;box-shadow:0 0 0 7px #00d9ff,0 0 42px #00d9ff;font-weight:900;cursor:pointer"><span style="font-size:9px;display:block">★ NEW</span>AUDIT TRAIL</button>':'')+'</div></section></div>';document.querySelector(".back").onclick=home;document.querySelector("#orbitCenter").onclick=()=>openPulse(role);document.querySelectorAll(".module").forEach(b=>b.onclick=()=>openModule(role,b));const ao=document.querySelector("#auditOrbit");if(ao)ao.onclick=openAudit}
...
function openPulse(role){
 ...
 app.innerHTML='<div class="shell"><section class="card workspace"><button class="back orbit-back">← Back to '+role+' Orbit</button><div class="workspace-circle module-demo pulse-view"><small>'+role.toUpperCase()+' • ORBIT CENTER</small><h2>'+data.title+'</h2><div class="pulse-grid">'+data.items.map(x=>'<div><small>'+x[0]+'</small><b>'+x[1]+'</b></div>').join("")+'</div>'+(role==="Owner"?'<button class="snapshot-action" id="openNotifications" type="button"><strong>Notification Center</strong><small>3 alerts need attention</small></button><button class="snapshot-action audit-new" id="openAudit" type="button" style="display:block;width:calc(100% - 54px);max-width:420px;margin:12px auto;padding:14px 16px;background:#00d9ff;color:#00131a;border:5px solid #fff;border-radius:18px;box-shadow:0 0 0 7px #00d9ff,0 0 42px #00d9ff;font-weight:900;position:relative;z-index:50"><span style="display:block;font-size:10px;margin-bottom:4px">★ NEW</span><strong style="display:block;font-size:17px">Audit Trail</strong><small style="display:block;color:#073047;margin-top:4px">View business activity history</small></button>':'')+'<p>'+data.note+'</p></div></section></div>';
