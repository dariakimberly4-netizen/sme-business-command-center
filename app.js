...
function portal(role){let d=roles[role];app.innerHTML='<div class="shell"><section class="card portal-orbit-only"><button class="back floating-back">← Roles</button><div class="portal-label"><small>SME COMMAND CENTER</small><h2>'+role+' Portal</h2></div><div class="orbit"><div class="center"><div><strong>'+d.center+'</strong><br><span>'+d.summary+'</span></div></div>'+d.modules.map((m,i)=>'<button class="module" data-m="'+m+'" style="'+pos(i,d.modules.length)+(role==="Owner"&&m==="Suppliers & Purchasing"?';background:#00d9ff;border:6px solid #ffffff;box-shadow:0 0 0 8px #00d9ff,0 0 55px #00d9ff;color:#00131a;font-weight:900;z-index:20':'')+'">'+m+(role==="Owner"&&m==="Suppliers & Purchasing"?'<span style="display:block;font-size:9px;margin-top:4px">★ NEW</span>':'')+'</button>').join("")+'</div></section></div>';document.querySelector(".back").onclick=home;document.querySelectorAll(".module").forEach(b=>b.onclick=()=>openModule(role,b))}
...
"Reports":'<div class="report-modules"><button class="report-module"><b>Daily Sales</b><span>₱48,650 • 126 transactions</span></button><button class="report-module"><b>Cash Flow</b><span>In ₱48,650 • Out ₱12,410 • Net ₱36,240</span></button><button class="report-module"><b>Profit & Loss</b><span>Revenue • COGS • Expenses • Net Profit</span></button><button class="report-module"><b>Inventory Movement</b><span>Stock In • Stock Out • Adjustments</span></button><button class="report-module"><b>Expenses</b><span>Today ₱12,410</span></button><button class="report-module"><b>Exceptions</b><span>2 items need review</span></button></div><button class="action">Export PDF</button> <button class="action">Export Excel</button>'
};
const roleExamples={
Manager:{
