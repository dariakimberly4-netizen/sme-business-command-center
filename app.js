...
function portal(role){let d=roles[role],st=loadState();if(role==="Owner"){d={...d,summary:money(st.sales)+" Today"}}app.innerHTML='<div class="shell"><section class="card portal-orbit-only"><button class="back floating-back">← Roles</button><div class="portal-label"><small>SME COMMAND CENTER</small><h2>'+role+' Portal</h2></div><div class="orbit"><button class="center center-action" id="orbitCenter" type="button"><div><strong>'+d.center+'</strong><br><span>'+d.summary+'</span><em>Tap for snapshot</em></div></button>'+d.modules.map((m,i)=>'<button class="module'+(role==="Cashier"&&m==="Shift Reconciliation"?' new-feature':'')+'" data-m="'+m+'" style="'+pos(i,d.modules.length)+'">'+m+(role==="Cashier"&&m==="Shift Reconciliation"?'<span class="new-badge">★ NEW</span>':'')+'</button>').join("")+(role==="Owner"?'<button id="ownerCloseOrbit" type="button" style="position:absolute;left:50%;top:67%;transform:translate(-50%,-50%);z-index:30;background:#00d9ff;color:#00131a;border:4px solid #fff;border-radius:999px;padding:8px 14px;box-shadow:0 0 0 5px #00d9ff,0 0 34px #00d9ff;font-weight:900;font-size:9px;cursor:pointer;white-space:nowrap"><span style="font-size:8px;margin-right:3px">★ NEW</span>END-OF-DAY</button>':'')+'</div></section></div>';document.querySelector(".back").onclick=home;document.querySelector("#orbitCenter").onclick=()=>openPulse(role);document.querySelectorAll(".module").forEach(b=>b.onclick=()=>openModule(role,b))};const oco=document.querySelector("#ownerCloseOrbit");if(oco)oco.onclick=()=>{let r=JSON.parse(localStorage.getItem("smeClosing")||"null");closingOwnerReview(r)}
