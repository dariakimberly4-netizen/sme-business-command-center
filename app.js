...
function openPulse(role){
 ...
 app.innerHTML='<div class="shell"><section class="card workspace"><button class="back orbit-back">← Back to '+role+' Orbit</button><div class="workspace-circle module-demo pulse-view"><small>'+role.toUpperCase()+' • ORBIT CENTER</small><h2>'+data.title+'</h2><div class="pulse-grid">'+data.items.map(x=>'<div><small>'+x[0]+'</small><b>'+x[1]+'</b></div>').join("")+'</div><p>'+data.note+'</p>'+(role==="Owner"?'<button class="action primary notification-new" id="openNotifications"><span class="new-badge">★ NEW</span> Open Notification Center</button>':'')+'</div></section></div>';
